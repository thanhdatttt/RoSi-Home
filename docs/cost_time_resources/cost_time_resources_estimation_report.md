...
- the expected cash budget for the current backlog is approximately **3.28 million VND**, excluding an unconfirmed runtime-AI feature;
- if the sponsor retains the runtime-AI feature from the old proposal, the working budget remains **4.25 million VND**;
- a conservative paid-fallback envelope is **up to approximately 5.3 million VND**, subject to actual provider and infrastructure quotations.
...
Backend coding speed does not eliminate the final integration and acceptance phases. This is why the full MVP estimate remains 8–10 weeks even though individual backend stories may be generated in minutes or hours.
...
| Cost category | Lean/free-tier | Expected current backlog | Sponsor proposal with runtime AI |
|---|---:|---:|---:|
| AI development tools | 0 while benefits remain | 1,600,000 | 1,600,000 |
| Runtime AI/API | 0; not in current backlog | 0 pending scope approval | 900,000 |
| Cloud infrastructure | 0 using approved credits | 800,000 | 800,000 |
| Domain/security | 450,000 | 450,000 | 450,000 |
| Contingency | 500,000 | 427,500 (15% of base) | 500,000 |
| **Estimated cash total** | **950,000** | **3,277,500** | **4,250,000** |

The expected scenario removes the 900,000 VND runtime-AI item because the current Product Backlog does not contain the corresponding generative-AI product feature. The sponsor must either approve that feature in the SOW or remove/reallocate the amount.

A conservative paid-fallback envelope of approximately **5.3 million VND** may be held for planning, calculated as the 4.25M sponsor proposal plus approximately 25% exposure for expired benefits, provider variance and infrastructure overage. It is a reserve ceiling, not approved spend.
...
| Cash budget | 4.25M VND | 3.28M current-scope expected; 4.25M if runtime AI retained | Confirm runtime-AI scope and contingency |
...
> We decomposed the backend into 51 user stories and calibrated member-specific rates using measured or declared AI-assisted work. We then forecast the remaining stories under optimistic, expected and conservative assumptions. We do not equate token count with cost or code completion with delivered value.
...
- unapproved runtime generative-AI features;
...
4. frontend and integration cycle-time data;
...
9. confirmed runtime-AI scope;
10. signed SOW baseline.
...
| Budget with sponsor runtime-AI scope | 4.25M VND |
| Conservative cash envelope | Up to approximately 5.3M VND |
