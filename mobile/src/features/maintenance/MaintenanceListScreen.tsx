...
export function MaintenanceListScreen({ tenant }: { tenant: boolean }) {
  ...
  return <MobileFrame><View style={{ flex: 1, backgroundColor: "#f5f8ff" }}>
    <ScreenHeader eyebrow={tenant ? "Tenant" : "Landlord"} title="Maintenance" />
    <View style={{ paddingHorizontal: 24, flexDirection: "row", gap: 8 }}>
      {filters.map((f) => <TouchableOpacity key={f} onPress={() => setStatus(f)} style={{ paddingHorizontal: 10, paddingVertical: 7, borderRadius: 999, backgroundColor: status === f ? "#2563eb" : "#e2e8f0" }}><Text style={{ color: status === f ? "white" : "#475569", fontSize: 11, fontWeight: "700" }}>{f === "InProgress" ? "In progress" : f}</Text></TouchableOpacity>)}
    </View>
    <ContentState loading={loading} error={error} empty={!loading && !error && items.length === 0} emptyMessage="No maintenance requests match this filter." />
    {!loading && !error ? <ScrollView contentContainerStyle={{ padding: 24, gap: 12, paddingBottom: 100 }}>
      {items.map((item) => <TouchableOpacity key={item.id} onPress={() => router.push({ pathname: `${base}/[id]` as any, params: { id: item.id } })} style={{ backgroundColor: "white", borderWidth: 1, borderColor: "#e2e8f0", padding: 16, borderRadius: 16 }}>
        ...
        <Text style={{ color: "#64748b", marginTop: 8 }}>{item.room.name} · {new Date(item.submittedAt).toLocaleDateString()}</Text>
      </TouchableOpacity>)}
    </ScrollView> : null}
    {tenant ? <TouchableOpacity accessibilityLabel="New maintenance request" onPress={() => router.push(`${base}/new` as any)} style={{ position: "absolute", right: 24, bottom: 28, width: 56, height: 56, borderRadius: 28, backgroundColor: "#2563eb", alignItems: "center", justifyContent: "center" }}><Plus color="white" /></TouchableOpacity> : null}
  </View></MobileFrame>;
}
