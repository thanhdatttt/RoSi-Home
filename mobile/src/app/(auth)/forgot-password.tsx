  ...
  async function submit() {
    ...
    try {
      ...
      if (e instanceof ApiRequestError) {
        setApiError(e.message || "Failed to request a new password.");
      } else {
      ...
      <View style={{ flex: 1, backgroundColor: '#f5f8ff' }}>
        ...
        <ScrollView style={{ flex: 1, paddingHorizontal: 24 }} contentContainerStyle={{ paddingBottom: Math.max(insets.bottom + 24, 24) }}>
          ...
          <Text style={{ fontSize: 14, color: '#94a3b8', marginTop: 8, lineHeight: 20 }}>
            Enter the email linked to your RosiHome account. If it matches an account, we'll email a new temporary password — we won't confirm either way for security.
          </Text>
