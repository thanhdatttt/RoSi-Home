  ...
  useFocusEffect(
    useCallback(() => {
      async function loadTenants() {
        if (!token) return;
        try {
          const data = await apiRequest<any[]>('/tenants?pageSize=100', { token });
          setTenants(data);
        } catch (err) {
          ...
          {loading ? (
            ...
            <View style={{ borderRadius: 16, borderWidth: 1, borderStyle: 'dashed', borderColor: '#e2e8f0', padding: 32, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ fontSize: 14, color: '#94a3b8', textAlign: 'center' }}>
                {q ? `No tenants match "${q}".` : "You haven't added any tenants yet."}
              </Text>
            </View>
          ) : (
            items.map((t) => (
              <View key={t.id} style={{ borderRadius: 16, borderWidth: 1, borderColor: '#e2e8f0', backgroundColor: '#ffffff', padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                ...
                <View style={{ flex: 1, paddingRight: 8 }}>
                  ...
                  <Text style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }} numberOfLines={1}>{t.email || "No email"}</Text>
                </View>
