...
export default function UploadPaymentProofScreen() {
  ...
  return <MobileFrame><View style={{ flex: 1, backgroundColor: "#f5f8ff" }}>
    <ScreenHeader eyebrow="US-PAYMENT-01" title="Upload payment proof" />
    ...
    {invoice ? <ScrollView contentContainerStyle={{ padding: 24, gap: 16, paddingBottom: 40 }}>
      <View style={{ padding: 16, borderRadius: 16, backgroundColor: "white", borderWidth: 1, borderColor: "#e2e8f0" }}>
        <Text style={{ color: "#64748b", fontSize: 12 }}>Invoice {invoice.billingPeriod}</Text>
        <Text style={{ color: "#0f172a", fontSize: 24, fontWeight: "800", marginTop: 4 }}>{vnd(invoice.totalAmount)}</Text>
        {invoice.roomName ? <Text style={{ color: "#64748b", marginTop: 4 }}>{invoice.roomName}</Text> : null}
      </View>
      {uploaded ? <View style={{ padding: 22, borderRadius: 18, backgroundColor: "#dcfce7", alignItems: "center", gap: 10 }}><CheckCircle2 size={40} color="#16a34a" /><Text style={{ color: "#166534", fontSize: 18, fontWeight: "800" }}>Proof submitted</Text><Text style={{ color: "#166534", textAlign: "center", lineHeight: 20 }}>Your landlord will compare it with the bank transaction before confirming payment.</Text></View> : <>
        <View style={{ padding: 14, borderRadius: 14, backgroundColor: "#eff6ff", flexDirection: "row", gap: 10 }}><ShieldCheck size={20} color="#2563eb" /><Text style={{ flex: 1, color: "#1e40af", lineHeight: 19 }}>Uploading proof does not mark this invoice Paid. Landlord verification is still required.</Text></View>
        {invoice.status !== "Sent" ? <View style={{ padding: 14, borderRadius: 12, backgroundColor: "#fee2e2" }}><Text style={{ color: "#b91c1c" }}>Only an unpaid Sent invoice accepts payment proof.</Text></View> : null}
        {file ? <View style={{ backgroundColor: "white", padding: 12, borderRadius: 16, borderWidth: 1, borderColor: "#e2e8f0" }}><Image source={{ uri: file.uri }} contentFit="contain" style={{ width: "100%", height: 280, borderRadius: 12, backgroundColor: "#f1f5f9" }} /><View style={{ flexDirection: "row", alignItems: "center", marginTop: 10, gap: 8 }}><Text numberOfLines={1} style={{ flex: 1, color: "#475569" }}>{file.name}</Text><TouchableOpacity accessibilityLabel="Remove payment proof" onPress={() => setFile(null)}><X size={20} color="#ef4444" /></TouchableOpacity></View></View> : <TouchableOpacity disabled={invoice.status !== "Sent"} onPress={chooseFile} style={{ height: 180, borderRadius: 18, borderWidth: 2, borderStyle: "dashed", borderColor: "#93c5fd", alignItems: "center", justifyContent: "center", backgroundColor: "white", opacity: invoice.status === "Sent" ? 1 : 0.5 }}><ImagePlus size={34} color="#2563eb" /><Text style={{ fontWeight: "700", color: "#2563eb", marginTop: 10 }}>Choose payment proof</Text><Text style={{ color: "#94a3b8", fontSize: 12, marginTop: 4 }}>PNG/JPG/JPEG · maximum 5 MB</Text></TouchableOpacity>}
        {error ? <Text style={{ color: "#b91c1c" }}>{error}</Text> : null}
        <PrimaryButton disabled={!file || uploading || invoice.status !== "Sent"} onPress={submit}>{uploading ? "Uploading..." : "Submit proof"}</PrimaryButton>
      </>}
      {uploaded ? <PrimaryButton variant="outline" onPress={() => router.replace({ pathname: "/(dashboard)/tenant/invoices/[id]", params: { id } } as any)}>Back to invoice</PrimaryButton> : null}
    </ScrollView> : null}
  </View></MobileFrame>;
}
